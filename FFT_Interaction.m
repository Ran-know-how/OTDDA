...
%生成扩展后的矩阵格子
%扩展的过程看论文《MATLAB package for discrete dipole approximation by graphics processing unit: Fast Fourier Transform and Biconjugate Gradient》
if GPU==1
...
%========== Calculating FFT in X-direction 计算X方向上的FFT
...
%========== Calculating FFT in Y-direction 计算Y方向上的FFT
% FFT in Matlab calculate DFT in x-direction, so for performing FFT ...Matlab中的FFT计算x方向上的DFT，以便执行FFT
% ... in y-direction, first we permute x and y lines, peform the ...在y方向上，首先我们排列x和y线，形成
% ... fft and again perform permutation to return back to first ...fft并再次执行置换以返回第一
% ... matrices dimension矩阵维数


AXX=fft(AXX(:,:,1:Nz),[],2); % Calculating FFT in y-direction计算y方向上的FFT
...
%======== Expanding dimension of each line in Z-direction to 2Nz-1 =======%将Z方向上每条线的尺寸扩展为2Nz-1
%=========================================================================%
...
%========== Calculating FFT in Z-direction 计算Z方向上的FFT
