...
%=========================================================================%
function [Outside_Index]=Excluding_NPs(x_plane,y_plane,z_plane,N_plane,Np_shape,Lx,Ly,Lz,d_inter,Structure,arrangement,theta)
...
%             (z_plane-Z0).^2/((Lz/2)^2))<=1));
                Index_in=find((sqrt((x_plane-X0).^2/((Lx/2)^2)+((y_plane-Y0)*cos(theta)+((z_plane-Z0)*sin(theta))).^2/((Ly/2)^2)+...
            ((z_plane-Z0)*cos(theta)-(y_plane-Y0)*sin(theta)).^2/((Lz/2)^2))<=1));
